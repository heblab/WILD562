...
packages <- c("ks", "lattice", "plotrix", "adehabitatHR", "maptools", "foreign", "rgdal", "sp", "raster", "ggplot2","colorRamps","rgeos")
...
##############################################################################################################################
# ggplot of elc habitat based on Elk Winter (ELK_W)
...
## lets make sure to look at the head of what we just created and make sure we take the time to understand all of these steps
head(elc_habitatDF)

#construct ggplot2 plot of Elk Winter Habitat
elk_plot<-ggplot(elc_habitatDF, aes(x = long, y = lat,group=group, fill = as.factor(ELK_W))) + 
...
elk_plot2 <- elk_plot + scale_fill_manual(name="ELK_W",values=c("black","gray40", "gray80","yellow", "orange", "red","darkblue"))
elk_plot2
...
## So lets bring it back in
disthhu2<-raster("DistFromHighHumanAccess2.tif") #resampled
plot(disthhu2)
...
########################################################
# calculate 99% minimum convex polygon for individual wolves in Red Deer wolf pack using the adehabitatHR package
# type ?mcp for more details about the mcp() estimator. 
...
# OBJECTIVE 3 - Learn how to sample availabiliity within home ranges 
...
head(wolfused)
write.table(wolfused, file = "/Users/mark.hebblewhite/Box Sync/Teaching/UofMcourses/WILD562/Spring2019/Labs/lab2/wolfused.csv", row.names=FALSE, na="", col.names=TRUE, sep=",")
wolfused <-read.csv("/Users/mark.hebblewhite/Box Sync/Teaching/UofMcourses/WILD562/Spring2019/Labs/lab2/wolfused.csv", header = TRUE)
...
par(mfrow = c(2,3))
multhist(list(wolfused$elk_w2[wolfused$pack=="Bow Valley"],wolfused$elk_w2[wolfused$pack=="Red Deer"]), freq = TRUE, main = "Cover Distance", legend.text = c("Bow Valley", "Red Deer"), args.legend = list(bty = "n"))
multhist(list(wolfused$deer_w[wolfused$pack=="Bow Valley"],wolfused$deer_w[wolfused$pack=="Red Deer"]), freq = TRUE, main = "Cover Distance", legend.text = c("Bow Valley", "Red Deer"), args.legend = list(bty = "n"))
multhist(list(wolfused$moose_w2[wolfused$pack=="Bow Valley"],wolfused$moose_w2[wolfused$pack=="Red Deer"]), freq = TRUE, main = "Cover Distance", legend.text = c("Bow Valley", "Red Deer"), args.legend = list(bty = "n"))
multhist(list(wolfused$sheep_w2[wolfused$pack=="Bow Valley"],wolfused$sheep_w2[wolfused$pack=="Red Deer"]), freq = TRUE, main = "Cover Distance", legend.text = c("Bow Valley", "Red Deer"), args.legend = list(bty = "n"))
multhist(list(wolfused$goat_w2[wolfused$pack=="Bow Valley"],wolfused$goat_w2[wolfused$pack=="Red Deer"]), freq = TRUE, main = "Cover Distance", legend.text = c("Bow Valley", "Red Deer"), args.legend = list(bty = "n"))
