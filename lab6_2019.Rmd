...
# Evaluating Predictions from Residual P
...
```
This reveals a LOT of information - lets compare these values to the table in the ? confusionMatrix help file, and here int his table. 

![Figure 6.1. Calculations of different quantities from a Confusion Matrix ](/Users/mark.hebblewhite/Box Sync/Teaching/UofMcourses/WILD562/Spring2019/Labs/lab6/ConfusionMatrix.png)
...
The receiver operating characteristic curve, i.e., ROC curve, is a graphical plot that illustrates the diagnostic ability of a binary classifier system as its discrimination threshold is varied.
...
-\infty  to the discrimination threshold) of the detection probability in the y-axis versus the cumulative distribution function of the false-alarm probability on the x-axis.
...
This plot shows the trade off between the True Positive Rate versus the False Positive Rate for our top model. Again, take a minute to reflect what this means, and then we will proceed to calculate the area under the curve, or, the AUC
...
Next, we will Plot ROC Curve with cut point and AUC
```{r}
...
Mapping JUST the numerator from the RSF from a Used-Available Design. For reason we will discuss in class, lets just map the numerator of the RSF for the biotic model just for comparative reasons for now. 
```{r}
...
a.	Re-run the RSF’s for just the Bow Valley pack.

b.	With the results of the Bow Valley pack RSF ‘stored’ – i.e., in active memory [recall in R the last model run is what is ‘active’] use this model to predict the probability of use for the Red Deer pack, and vice versa. 

c.	Conduct model evaluation using the R-squared from a linear regression of the prediction for the Bow Valley pack using the Bow Valley model vs. the Red Deer Model, and vice versa.

d.	Can you think of how to do k-folds cross validation between packs? i.e., compare the area adjusted frequency of counts in each of 10 ranked deciles to that predicted for the other pack??? What does the k-folds cross validation tell us about differences between predictions at the population level and predictions at the pack-level??? Follow the lab instructions for manually calculating k-folds above using the excel spreadsheets.
