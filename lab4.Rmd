...
```{r}
source("/Users/mark.hebblewhite/Box Sync/Teaching/UofMcourses/WILD562/Spring2019/Labs/lab4/new/Lab2NeededforLab5.R", verbose = FALSE)
```
...
First we will estimate Manly's selection ratio by comparing the ratio of used and available locations to each other, and taking the natural logarithm of those in R. Manly et al. (2002) provide a methodology for this kind of design, relying on the calculation of selection ratios:

$$\omega_j = \upsilon_j*\alpha_i$$
where $\upsilon_j$ is the proportion of use of the habitat class __j__ and $\alpha_i$ is the proportion of availability of this habitat class __j__.
Note that these ratios may be scaled so that their sum is equal to 1, that is:
$$\beta_j = \frac{\omega_j}{\sum_{i=1}^j \alpha_i}$$
where $\beta_j$ is said to be the Manly Selectivity Coefficient for habitat class __j__. We will see below when we perform logistic regression on this same categorical dataset the relationship between Manly's selectivity coefficient, $\beta_j$,  and that estimated from logistic regression. 

Note that it is customary to take the natural logarithmic transformation of the $\omega_j$ to make the selectivity index symmetric for the same amount of increase or decrease of use or availability. This is a similar property in population ecology to the difference between lambda, the ratio of two population sizes which estimates the absolute growth rate, to little r, or intrinsic population growth rate, which measures the relative growth rate. So we have 
$$\ln(\omega_j) = \upsilon_j*\alpha_i$$
This is often referred to as the ln of the selection ratio, or the Selection index. 
...
[link](https://rdrr.io/cran/adehabitat/man/wi.html) 
[link](http://www2.uaem.mx/r-mirror/web/packages/adehabitatHS/vignettes/adehabitatHS.pdf) 
```{r}
